...
void main()
{
 int max=308 ;                                     // maximum value tempreture sensor can detect
...
//    WordToStr(key, key_txt);d
  ...
  while(1){
           ...
           IntToStr(temp,  result);      // integer value temp will be converted to string to be stored in result
...
//
    ...
    else if (temp >=thresh && temp >=max)      // if tempreture greater than thresh and greater than max                                                // OTHERWISE
    {
      ...
      Delay_ms(1000) ;
      PORTB = 0b01010101 ;
      Delay_ms(1000) ;                                               // PORT D HALF OF & HALF ON
      //
      //
